...
If any of the additionally added hosts do not have a properly configured DNS, Let's Encrypt will not be able to renew the certificate for any of the domains, including the main domain. If you're using additional hosts and facing https issues with the main domain, try removing a domain from the `BTCPAY_ADDITIONAL_HOSTS` domain and re-run the setup. The https issue also occurs if [Dynamic DNS](DynamicDNS.md) has not been renewed and is an additional host.
