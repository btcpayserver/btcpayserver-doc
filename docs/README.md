---
layout: Homepage
heroImage: /img/btcpay-logo.svg
heroText: BTCPay Server Documentation
tagline: Start Accepting Bitcoin Payments With 0% Fees & No Third-party
features:
  - title: Users
    details: Accept and manage Bitcoin payments with BTCPay Server.
    actionText: User Docs  ❯
    actionLink: /Users/
  - title: Operators
    details: Deploy, configure, and maintain a BTCPay Server instance.
    actionText: Operator Docs  ❯
    actionLink: /Operators/
  - title: Developers
    details: Build integrations and plugins for BTCPay Server.
    actionText: Developer Docs  ❯
    actionLink: /Developers/
footer: Content Released under MIT license.
---

<div class="topics">
  <h3>Explore by topic</h3>

- [What is BTCPay&nbsp;Server](./Users/overview.md)
- [Why BTCPay&nbsp;Server](./UseCase.md)
- [How to deploy BTCPay&nbsp;Server](./TryItOut.md)
- [Wallet Setup](./Users/wallet-setup.md)
- [Lightning Network](./LightningNetwork.md)
- [How to setup POS](./Conference-PoS-guide.md)
- [Greenfield API](./Developers/api/README.md)
- [Frequently Asked Questions<br>and Common Issues](./FAQ/README.md)
- [How to contribute to BTCPay&nbsp;Server](./Contribute/README.md)

</div>

!!!include(supporters.html)!!!
