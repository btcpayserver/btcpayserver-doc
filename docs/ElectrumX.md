...
2. Unpruned BTCPay node: Make sure your BTCPay implementation is NOT [pruned](./FAQ/Synchronization.md#can-i-skip-the-synchronization) (i.e. you have synced and stored from the genesis block. Check that you do NOT use an `opt-save-storage` [fragment](/Docker/configuration/#storage-and-memory-profiles)).
...
4. Additional Fragments: You are familiar with how to use BTCPay's [optional fragments](/Docker/fragments/) as part of your environment variable setup.
...
2. Enable the Docker Additional Fragment on your BTCPay node by running the following commands (this is assuming a brand new BTCPay installation with LND and ElectrumX, please tweak accordingly using the [relevant documentation](/Docker/fragments/)):

3. Follow the [normal setup and installation of BTCPay Server](/Docker/installation/), then after `cd btcpayserver-docker`, follow the instructions below instead. If you already have a BTCPay Server running, continue from the next step.
